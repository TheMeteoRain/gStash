...
const queries = {
  ...
  insertProperties: (data: Property[]) => pgp.helpers.insert(data, COLUMN_SET_PROPERTIES),
  ...
  removeStashes: (data: Stash[]) => pgp.helpers.concat(data.map((stash: Stash) => REMOVE_STASH_BY_ID(stash.stash_id))),

  concat: (queries: string[]): string => pgp.helpers.concat([...queries]),

  upsertCurrentNextChangeId: (next_change_id: string, downloaded: boolean = false): Promise<any> =>
    db.one(`
    INSERT INTO changeid(next_change_id, downloaded) VALUES($<next_change_id>, $<downloaded>)
    ON CONFLICT (next_change_id)
    DO UPDATE SET downloaded = EXCLUDED.downloaded
    RETURNING next_change_id`, { next_change_id, downloaded: false }),
